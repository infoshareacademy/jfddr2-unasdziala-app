      ...
      <Switch>
        <Route exact path={path}>
          ...
          {userServiceList.map((userService) => {
              return <UserService key={userService.id}  userService={userService} setDummyVariable={setDummyVariable}/>;
            })}
          <Link to="/">
        ...
        <Route path={`${path}/:userServiceTitle`}>
          <ModifyService setDummyVariable={setDummyVariable} userLogInState={userLogInState} userServiceList={userServiceList} />
        </Route>
      </Switch>
    </div>
  );
